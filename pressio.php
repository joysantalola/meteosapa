...
<div class="container">
    ...
    <h2>Pressió atmosfèrica</h2>

    <div class="introduccio">
        <h3>Consulta de la pressió atmosfèrica</h3>

        <p>
            En aquesta pàgina pots consultar les dades de pressió atmosfèrica
            registrades per l’estació.
        </p>

        <p>
            La pressió atmosfèrica és un indicador important per a la predicció
            meteorològica i permet identificar canvis en el temps.
        </p>

        <p>
            A partir de la data seleccionada, es mostren els valors màxims, mínims
            i mitjans enregistrats durant el dia.
        </p>
    </div>
    ...
    <?php if ($data): ?>
        <?php if ($resultats && $resultats['max'] !== null): ?>
            ...
            <div class="resultats">
                ...
                <p>Màxima: <strong><?= $resultats['max'] ?> hPa</strong></p>
                <p>Mínima: <strong><?= $resultats['min'] ?> hPa</strong></p>
                <p>Mitjana: <strong><?= round($resultats['mitja'], 2) ?> hPa</strong></p>
            </div>
        <?php else: ?>
            <div class="error">
