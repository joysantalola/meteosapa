...
<div class="container">
    ...
    <div class="introduccio">
        <h3>Consulta de la humitat</h3>

        <p>
            Aquesta secció permet visualitzar el nivell d’humitat relativa de l’aire
            registrat per l’estació meteorològica.
        </p>

        <p>
            Seleccionant una data concreta, es poden consultar els valors màxims,
            mínims i mitjans de la humitat durant aquell dia.
        </p>

        <p>
            La humitat és un factor clau per entendre la sensació tèrmica i les
            condicions ambientals generals.
        </p>
    </div>
    ...
    <?php if ($data): ?>
        <?php if ($resultats && $resultats['max'] !== null): ?>
            <div class="resultats">
                ...
                <p>Màxima: <strong><?= $resultats['max'] ?> %</strong></p>
                <p>Mínima: <strong><?= $resultats['min'] ?> %</strong></p>
                <p>Mitjana: <strong><?= round($resultats['mitja'], 2) ?> %</strong></p>
            </div>
